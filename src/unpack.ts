import type { Arr, Key, Pointer, Struct, Unpacked, numbers } from './types.js';
...
export class UnpackPointer extends SharedPointer implements Pointer {
	...
	u16(key: Key<number | numbers>, length?: number): number | numbers {
		...
		if (length === undefined) {
			value = this._ctx.view.getUint16(this._pos, this._little);
			this._pos += 2;
		}
		else {
			value = new Uint16Array(length);
			for ( let i=0; i<length; i++ ) value[i] = this._ctx.view.getUint16(this._pos + i*2, this._little);
			this._pos += length * 2;
		}
	...
	u32(key: Key<number | numbers>, length?: number): number | numbers {
		...
		if (length === undefined) {
			value = this._ctx.view.getUint32(this._pos, this._little);
			this._pos += 4;
		}
		else {
			value = new Uint32Array(length);
			for ( let i=0; i<length; i++ ) value[i] = this._ctx.view.getUint32(this._pos + i*4, this._little);
			this._pos += length * 4;
		}
	...
	i16(key: Key<number | numbers>, length?: number): number | numbers {
		...
		if (length === undefined) {
			value = this._ctx.view.getInt16(this._pos, this._little);
			this._pos += 2;
		}
		else {
			value = new Int16Array(length);
			for ( let i=0; i<length; i++ ) value[i] = this._ctx.view.getInt16(this._pos + i*2, this._little);
			this._pos += length * 2;
		}
	...
	i32(key: Key<number | numbers>, length?: number): number | numbers {
		...
		if (length === undefined) {
			value = this._ctx.view.getInt32(this._pos, this._little);
			this._pos += 4;
		}
		else {
			value = new Int32Array(length);
			for ( let i=0; i<length; i++ ) value[i] = this._ctx.view.getInt32(this._pos + i*4, this._little);
			this._pos += length * 4;
		}
	...
	f32(key: Key<number | numbers>, length?: number): number | numbers {
		...
		if (length === undefined) {
			value = this._ctx.view.getFloat32(this._pos, this._little);
			this._pos += 4;
		}
		else {
			value = new Float32Array(length);
			for ( let i=0; i<length; i++ ) value[i] = this._ctx.view.getFloat32(this._pos + i*4, this._little);
			this._pos += length * 4;
		}
	...
	f64(key: Key<number | numbers>, length?: number): number | numbers {
		...
		if (length === undefined) {
			value = this._ctx.view.getFloat64(this._pos, this._little);
			this._pos += 8;
		}
		else {
			value = new Float64Array(length);
			for ( let i=0; i<length; i++ ) value[i] = this._ctx.view.getFloat64(this._pos + i*8, this._little);
			this._pos += length * 8;
		}
