...
export class PackPointer<I extends Unpacked = Unpacked> extends SharedPointer implements Pointer<I> {
	...
	#get_array_value(key: Key<I, any>, length: number): ArrayLike<any> {
		...
		if (v.length !== length) throw TypeError(`${this.context.name}: Expected array of length ${length} for key ${key.toString()}, but got ${v.length} instead!`);
		return v;
	}
	...
	u8(key: Key<I, number>,  length?: number): number | Uint8Array {
		if (length === undefined) {
			const value = this.#get_single_value(key, 'number');
	...
	u32(key: Key<I, number>, length?: number): number | Uint32Array {
		...
		const value = new Uint32Array(this.#get_array_value(key, length));
		const start = this.position;
	...
	u64(key: Key<I, bigint>, length?: number): bigint | BigUint64Array {
		if (length === undefined) {
			const value = this.#get_single_value(key, 'bigint');
			this.context.view.setBigUint64(this.position, value, this.little);
			this.position += 8;
			return value;
		}

		const value = new BigUint64Array(this.#get_array_value(key, length) as BigUint64Array);
		const start = this.position;
	...
	i8(key: Key<I, number>, length?: number): number | Int8Array {
		...
		const value = new Int8Array(this.#get_array_value(key, length));
		this.context.array.set(value, this.position);
		this.position += length;
		return value;
	}
	...
	i32(key: Key<I, number>, length?: number): number | Int32Array {
		...
		const value = new Int32Array(this.#get_array_value(key, length));
		const start = this.position;
		this.position += length * 4;
	...
	i64(key: Key<I, bigint>, length?: number): bigint | BigInt64Array {
		...
		const value = new BigInt64Array(this.#get_array_value(key, length) as BigInt64Array);
		const start = this.position;
		this.position += length * 8;
	...
	f16(key: Key<I, number>, length?: number): number | Float16Array {
		if (length === undefined) {
			const value = this.#get_single_value(key, 'number');
			this.context.view.setFloat16(this.position, value, this.little);
			this.position += 2;
			return value;
		}

		const value = new Float16Array(this.#get_array_value(key, length));
		const start = this.position;
		this.position += length * 2;
		for (let i=0; i<value.length; i++) this.context.view.setFloat16(start + i*2, value[i], this.little);
		return value;
	}
	...
	f32(key: Key<I, number>, length?: number): number | Float32Array {
		...
		const value = new Float32Array(this.#get_array_value(key, length));
		const start = this.position;
		this.position += length * 4;
	...
	f64(key: Key<I, number>, length?: number): number | Float64Array {
		if (length === undefined) {
			const value = this.#get_single_value(key, 'number');
			this.context.view.setFloat64(this.position, value, this.little);
			this.position += 8;
			return value;
		}

		const value = new Float64Array(this.#get_array_value(key, length));
		const start = this.position;
		this.position += length * 8;
	...
	str(key: SKey<I, string>, length?: number): string {
		const value = this.#get_single_value(key, 'string');
		...
		if (length === undefined) {
			...
			throw TypeError(`${this.context.name}: Expected a string of length ${length} for key ${<key>key}, but got ${value.length} instead!`);
		}
